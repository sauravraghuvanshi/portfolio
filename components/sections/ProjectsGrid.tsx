...
export default function ProjectsGrid({ projects, showFilters = true, limit }: ProjectsGridProps) {
  ...
  // Every project renders in the same card format — no per-id special cases.
  const regularProjects = displayed;
            ...
            {regularProjects.map((project, i) => (
              <Link key={project.id} href={`/projects/${project.id}`} className="block">
                ...
                >
                  ...
                  {/* Title/description/outcomes are all capped so a long record can't
                      stretch its grid row and leave its neighbours with dead space.
                      The full text lives on /projects/[slug]. */}
                  <h3 className="font-semibold text-slate-900 dark:text-white mb-2 line-clamp-2 min-h-[3rem]">{project.title}</h3>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-4 line-clamp-4 min-h-[5.75rem]">{project.description}</p>
                  ...
                  {project.outcomes && project.outcomes.length > 0 && (
                    <div className="mb-4 space-y-1">
                      ...
                      {project.outcomes.length > 3 && (
                        <p className="pl-5 text-xs text-slate-400 dark:text-slate-500">
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-4 mt-auto border-t border-slate-100 dark:border-slate-800">
