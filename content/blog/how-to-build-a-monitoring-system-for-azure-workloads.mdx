...
Azure Workloads (App Service / AKS / Functions / VMs / APIs)
→ Instrumentation (Structured Logs, Traces, Metrics)
→ Azure Monitor
→ Application Insights, Log Analytics Workspace, Azure Monitor Metrics
→ Workbooks & KQL Analysis, Metric Alerts, Log Alerts
→ Action Groups
→ Email / Teams / Webhook / Automation

Specialist agents (VM Insights, Container Insights, Network Watcher) feed directly into Log Analytics alongside the core workload telemetry.
