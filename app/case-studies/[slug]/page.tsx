...
export default async function CaseStudyPage({ params }: CaseStudyPageProps) {
  ...
  const { content } = await compileMDX({
    ...
    components: {
      ...
      h3: ({ children, ...props }) => (
        <h3 className="text-xl font-semibold text-slate-900 dark:text-white mt-8 mb-3 scroll-mt-24" {...props}>
          {children}
        </h3>
      ),
      h4: ({ children, ...props }) => (
        <h4 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mt-6 mb-2" {...props}>
          {children}
        </h4>
      ),
      p: ({ children, ...props }) => (
      ...
      ol: ({ children, ...props }) => (
        <ol className="list-decimal list-inside space-y-2 mb-6 text-slate-700 dark:text-slate-300" {...props}>
          {children}
        </ol>
      ),
      li: ({ children, ...props }) => (
        <li className="flex items-start gap-2 text-slate-700 dark:text-slate-300 text-sm leading-relaxed" {...props}>
          <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-brand-500 flex-shrink-0" aria-hidden="true" />
          <span>{children}</span>
        </li>
      ),
      ...
      pre: ({ children, ...props }) => (
        <pre className="bg-slate-900 dark:bg-slate-950 rounded-xl overflow-x-auto my-6 p-0" {...props}>
          {children}
        </pre>
      ),
